...
`).bind(Date.now()).run();
    ...
    if (path !== '/cleanup') {
      const now = Date.now();
      const expireThreshold = now - 24 * 60 * 60 * 1000; // 24小时前
    ...
    if (path === '/create' && request.method === 'POST') {
      ...
      await env.DB.prepare(`
        INSERT INTO rooms (id, host, host_token, created_at, expires_at) 
        VALUES (?, ?, ?, ?, ?)
      `).bind(id, nick, hostToken, now, expiresAt).run();
    ...
    if (path === '/state' && request.method === 'GET') {
      ...
      const member = await env.DB.prepare('SELECT nickname FROM members WHERE room_id = ? AND nickname = ? AND token = ?')
    ...
    if (path === '/message' && request.method === 'POST') {
      ...
      await env.DB.prepare('INSERT INTO messages (id, room_id, nickname, content, is_host, time) VALUES (?, ?, ?, ?, ?, ?)')
    ...
    // ── POST /api/cleanup （新增：供 Cron Trigger 调用，手动触发也可） ──
    if (path === '/cleanup' && request.method === 'POST') {
      const now = Date.now();
      const expireThreshold = now - 24 * 60 * 60 * 1000;

      const deletedRooms = await env.DB.prepare('SELECT id FROM rooms WHERE expires_at < ?').bind(expireThreshold).all();

      await env.DB.prepare('DELETE FROM messages WHERE room_id IN (SELECT id FROM rooms WHERE expires_at < ?)')
        .bind(expireThreshold).run();
      await env.DB.prepare('DELETE FROM members WHERE room_id IN (SELECT id FROM rooms WHERE expires_at < ?)')
        .bind(expireThreshold).run();
      await env.DB.prepare('DELETE FROM rooms WHERE expires_at < ?').bind(expireThreshold).run();

      return ok({ 
        success: true, 
        message: `已清理过期会议室`, 
        cleanedCount: deletedRooms.results.length 
      });
    }
