export async function onRequest(context) {
  ...
  try {
    ...
    if (path === '/create' && request.method === 'POST') {
      ...
      const now = Date.now();
      await env.DB.prepare('INSERT INTO rooms (id, host, host_token, created_at) VALUES (?, ?, ?, ?)').bind(id, nick, hostToken, now).run();
      await env.DB.prepare('INSERT INTO members (room_id, nickname, token, last_seen) VALUES (?, ?, ?, ?)').bind(id, nick, memberToken, now).run();
    ...
    if (path === '/join' && request.method === 'POST') {
      ...
      return ok({ roomId: id, nickname: nick, isHost, token: memberToken, hostToken: isHost ? room.host_token : undefined });
    }
    ...
    if (path === '/state' && request.method === 'GET') {
      ...
      const member = await env.DB.prepare('SELECT nickname FROM members WHERE room_id = ? AND nickname = ? AND token = ?').bind(roomId, nickname, token).first();
      if (!member) return err('身份验证失败', 401);
      const now = Date.now();
      if (now - member.last_seen > 120000) {
        await env.DB.prepare('UPDATE members SET last_seen = ? WHERE room_id = ? AND nickname = ?').bind(now, roomId, nickname).run();
      }
      const activeMembersResult = await env.DB.prepare('SELECT nickname FROM members WHERE room_id = ? AND last_seen > ?').bind(roomId, now - 180000).all();
      const messagesResult = await env.DB.prepare('SELECT * FROM messages WHERE room_id = ? ORDER BY time ASC').bind(roomId).all();
    ...
    if (path === '/message' && request.method === 'POST') {
      ...
      await env.DB.prepare('INSERT INTO messages (id, room_id, nickname, content, is_host, time) VALUES (?, ?, ?, ?, ?, ?)').bind(msgId, roomId, nickname, content.trim(), nickname === room.host ? 1 : 0, Date.now()).run();
    ...
    if (path === '/upload-file' && request.method === 'POST') {
      ...
      await env.DB.prepare('UPDATE rooms SET file_content = ?, file_name = ? WHERE id = ?').bind(content, fileName, roomId).run();
    ...
    if (path === '/pin' && request.method === 'POST') {
      ...
      if (!room) return err('房间不存在', 404);
      if (room.host !== nickname || room.host_token !== hostToken) return err('权限不足', 403);
      await env.DB.prepare('UPDATE rooms SET pinned_message_id = ? WHERE id = ?').bind(messageId, roomId).run();
      return ok({ success: true });
    }
    ...
    if (path === '/require-answer' && request.method === 'POST') {
      ...
      if (!room) return err('房间不存在', 404);
      if (room.host !== nickname || room.host_token !== hostToken) return err('权限不足', 403);
      await env.DB.prepare('UPDATE rooms SET required_members = ? WHERE id = ?').bind(JSON.stringify(members), roomId).run();
      return ok({ success: true });
    }
    ...
    if (path === '/clear-required' && request.method === 'POST') {
      ...
      await env.DB.prepare('UPDATE rooms SET required_members = ? WHERE id = ?').bind(JSON.stringify(updated), roomId).run();
      return ok({ success: true });
    }
    ...
    if (path === '/announcement' && request.method === 'POST') {
      ...
      if (!room) return err('房间不存在', 404);
      if (room.host !== nickname || room.host_token !== hostToken) return err('权限不足', 403);
      await env.DB.prepare('UPDATE rooms SET announcement = ?, announcement_time = ? WHERE id = ?').bind(content, Date.now(), roomId).run();
      return ok({ success: true });
    }

    // ── POST /api/clear-messages（新增：清空聊天记录）──────────
